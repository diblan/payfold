...
| Throughput at 330k/day | Measured end to end on the async spine — producer: 1,015,000 due rows scanned + published in 459 s wall, peak heap 183 MiB; consumer at conc-1: **111/s measured 2026-08-22 at 100k** on the [D24](decisions.md#d24) relay (drain 899 s, end-to-end completion 1,046 s including recovery and the full dunning arc; the 15k demo scale measures 195/s at ~6 ms/consume). A 330k night is ~2.5 min of publishing plus ~50 min of draining — **29× the 3.8/s requirement** at the single-thread default, before either scaling lever. Single-node WSL2 dev-laptop numbers (see quality.md "Measured scale runs") |
...
| Consumer scaling levers | Measured on the async spine, 2026-08-22 at 100k on the [D24](decisions.md#d24) relay — 3 same-host replicas at concurrency 1: drained in 388 s (258/s, ~2.3× baseline; RabbitMQ round-robin split 33,326/33,365/33,309), end-to-end settled in 474 s, full verify.sh-green with fleet-summed exact checks and three `SKIP LOCKED` relays sharing one inbox. Listener concurrency ×8: drained in 219 s (457/s; 618/s first-minute burst absorbed by the single relay at sub-3 s lag), settled in 529 s, zero dead-letters and zero submit timeouts. On one host in-process concurrency is the cheaper lever, and the binding constraint is now the shared substrate (one Postgres absorbing every write; the per-JVM settlement side draining the burst's backlog); the retired 524/s figure was WireMock-era. Same-host replicas contend for one machine; true multi-node scaling is the external platform repo's KEDA demonstration |
...
| `DUNNING_RETRIABLE_GRACE_SECONDS` (consumer; yaml `dunning.retriable-grace-seconds`) | `DunningProperties`, `DunningLifecycle`; compose overrides the 604800 s application default with 600 s for verify/demo (bounded exhaustion demonstrably beats the expiry backstop at the compose cadence — [D21](decisions.md#d21), margin re-widened by [D23](decisions.md#d23) at 15k and again by [D25](decisions.md#d25) after the 100k exhaustion-path tail measured 205 s max against the 180 s backstop) | alive |
