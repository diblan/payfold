...
| `payment-service/renewal-consumer` | **A** | Tested (real-broker integration suite including decline, timeout, poison, and payment-method routing paths, on Testcontainers 2.x with no machine-local Docker pins), observable (SLF4J, `renewals_processed_total{outcome}`, Prometheus endpoint, and listener timer), and documented (contract + architecture); cards retain the existing synchronous PSP path while SDD submissions park unfinalized in `submitted`; dedicated tests prove bank errors exhaust the G5 retry cap into the DLQ and SDD redelivery makes no duplicate row or bank call; no known behavior defects | — |
| `db-migrations` | **B** | Clean, ordered, sole schema authority; since R18 also ships as the `payfold-migrations` Job image (SQL baked at build, FLYWAY_* env config), whose no-op re-run `verify.sh` asserts; V6 adds customer payment methods with constrained SDD debtor material and submitted-payment bank attribution; V1 carries aspirational tables (`bank_tx`, `recon_match`, `ledger_entry`) no code uses — harmless but reviewer-confusing | — |
...
the payment remains pending. The suite also covers the poison path: malformed and
contract-violating messages dead-letter while a subsequent good message processes.
