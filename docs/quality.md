...
| `payment-service/renewal-consumer` | **A** | Tested (real-broker integration suite including decline, timeout, poison, payment-method routing, and the full settlement spine on Testcontainers 2.x with no machine-local Docker pins), observable (SLF4J, eager renewal/webhook/settlement counters, Prometheus endpoint, listener timers, and settlement dashboard panels), and documented (contracts + architecture); cards retain the existing synchronous PSP path while SDD settlement runs through the HMAC webhook, durable inbox, confirm-gated relay, settlements queue/DLQ, and idempotent listener; tests cover duplicate webhook/message delivery, poison isolation, failed reasons, and deferred chargebacks; no known behavior defects | — |
| `db-migrations` | **B** | Clean, ordered, sole schema authority; since R18 also ships as the `payfold-migrations` Job image (SQL baked at build, FLYWAY_* env config), whose no-op re-run `verify.sh` asserts; V6 adds customer payment methods with constrained SDD debtor material and submitted-payment bank attribution; V7 adds the durable settlement inbox/outbox and terminal payment failure reason; V1 carries aspirational tables (`bank_tx`, `recon_match`, `ledger_entry`) no code uses — harmless but reviewer-confusing | — |
...
accepted collections park submitted without finalization, duplicate delivery skips
resubmission, and bank 5xx responses exhaust exactly five attempts into the DLQ while
the payment remains pending. The settlement spine suite covers signed receiver
semantics, durable webhook deduplication, confirm-relayed inbox rows, idempotent
settlement redelivery, terminal failure reasons, deferred chargebacks, and poison
settlements reaching their dedicated DLQ while good messages flow. The suite also
