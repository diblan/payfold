...
| `payment-service/renewal-consumer` | **D** | Billing chain works same-day, but: no actuator while compose healthchecks it (perpetually "unhealthy"), cross-midnight redelivery duplicates records, DLQ unreachable, dead PSP config, webflux-instead-of-web + duplicate deps in pom | [R1](roadmap.md#r1), [R2](roadmap.md#r2), [R4](roadmap.md#r4), [R5](roadmap.md#r5), [R8](roadmap.md#r8) |
...
| `docker-compose.yaml` + config | **C** | Stack boots and orders dependencies correctly; dotted env names bind only by accident, dead yaml keys, consumer healthcheck can never pass, RabbitMQ volume hardcoded to `/payfold/rabbitmq`, and `.env.example`'s `POSTGRES_VOLUME=pg_data` refers to an undeclared named volume — the documented first-time `docker compose up` fails until the value is changed to a path | [R1](roadmap.md#r1), [R2](roadmap.md#r2) |
