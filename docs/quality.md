...
| `payment-service/renewal-consumer` | **B** | The provider failure path is real: deterministic mock-PSP decline + timeout integration tests, with failed payments quarantined unfinalized and terminal. The remaining gap is `System.out` logging | [R9](roadmap.md#r9) |
...
| `docs/` + harness | **B** | CI uses pinned Maven wrappers and runs real-container integration tests for both services; `verify.sh` covers the happy path, same-day idempotency, poison probe, and per-row predicted PSP outcomes with an exact failed count | [R9](roadmap.md#r9) |
