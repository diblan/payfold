...
| `seed-data-gen` | **B** | Seed size parameterized (`SEED_CUSTOMERS`, default 15k, all due today); emails numbered from the current row count so `customer_email_key` cannot collide at any size; since R18 ships as the `payfold-seed-data-gen` Job image (source, JDBC driver, and name data baked; strict-exit runner); the documented 100k run seeds in ~5 s and passed verify.sh. Remaining gap: month-end clamp days | [R16](roadmap.md#r16) |
