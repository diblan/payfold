...
| `payment-service/renewal-consumer` | **A** | Tested (real-broker integration suite including decline, timeout, poison, payment-method routing, and the full settlement spine on Testcontainers 2.x with no machine-local Docker pins), observable (SLF4J, eager renewal/webhook/settlement counters, Prometheus endpoint, listener timers, and settlement dashboard panels), and documented (contracts + architecture); cards retain the existing synchronous PSP path while SDD settlement runs through the HMAC webhook, durable inbox, confirm-gated relay, settlements queue/DLQ, and idempotent listener; tests cover duplicate webhook/message delivery, poison isolation, failed reasons, and the idempotent, order-tolerant chargeback lifecycle through disputed invoice with no subscription compensation; no known behavior defects | — |
...
| `docs/` + harness | **B** | CI uses pinned Maven wrappers and runs real-container integration tests for both services; a tag-triggered workflow publishes the five multi-arch deploy images (R18); `verify.sh` covers the happy path via the async trigger (<1s POST assert + execution-status polling), same-day idempotency, poison probe, per-row predicted PSP outcomes with an exact failed count, same-run metric/DB delta cross-checks, and the migrations image's no-op Job re-run; since R17 every management-API depth assertion is a bounded poll (the last single-read check flaked at 100k on the API's ~5s stats interval and is fixed the way its DLQ siblings already were); since R20 the consumer metric checks sum across the replica port range and are proven green at 3 replicas; since R21 it also asserts Prometheus is scraping both services and Grafana serves the provisioned dashboard; `scripts/chaos-demo.sh` (R22/R23d) is a six-scene failure-mode demo whose two-cohort prediction now covers card and SDD outcomes and whose slow-bank scene proves a submitted plateau followed by MD06 chargebacks; R23d sharpens SDD verification to exact chargeback counts and asserts every charged-back invoice is disputed with no subscription rollback, alongside settlement-queue hygiene and row-for-row inbox reconciliation | — |
...
settlement redelivery, terminal failure reasons, chargebacks after settlement and
before settlement, idempotent chargeback redelivery, and poison settlements reaching
their dedicated DLQ while good messages flow. The suite also
