...
| `payment-service/renewal-consumer` | **A** | Tested (real-broker integration suite including synchronous card decline, authorization timeout to bounded DLQ, asynchronous card settlement/chargeback, poison isolation, deterministic BE→bank-a/NL→bank-b routing, per-counterparty webhook secrets, the shared settlement spine, and an advisory-locked, page-shaped recovery sweeper tested for inbox synthesis, 404 resubmission, freshness, and race dedupe, with DB polls that treat a missing row as retriable within their bounded windows, [R29](roadmap.md#r29)), observable (method-tagged eager renewal counters, webhook/settlement/recovery counters, per-counterparty latency timers, Prometheus, listener timers, dashboard panels), and documented; both methods use one registry/client and one durable inbox→queue→listener path, while stale submissions cannot be stranded by missing webhooks; no known behavior defects | — |
...
| `docker-compose.yaml` + config | **B** | Stack ordering and healthchecks pass; yaml contains only consumed keys; flyway and seed-data run the published image shapes; the same counterparty image runs bank-a, bank-b, and cardnet with a scheme-aware registry, deterministic silent-share seed settings, and verify/demo-fast recovery overrides; Prometheus/Grafana now expose recovery alongside the scale-safe consumer ports and confirm-gated settlement relay | — |
| `docs/` + harness | **B** | CI uses pinned Maven wrappers and real-container integration suites; `verify.sh` covers trigger/idempotency/poison behavior, exact token- and IBAN-predicted outcomes and reasons, method-tagged renewal counters, all-payment zero-stuck and inbox reconciliation, per-counterparty exact inbox shares, suffix-94 recovery outcomes/inbox provenance/counter exactness, health, and zero give-ups; `scripts/chaos-demo.sh` keeps eight asserted scenes, including live counterparty-amnesia recovery without manual intervention | — |
